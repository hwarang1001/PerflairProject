...
public class PaymentServiceImpl implements PaymentService {
	...
	public Long completePayment(PaymentRequestDTO requestDTO) {
		...
		// 3. 총 결제 금액 계산
		long totalAmount = cartItems.stream().mapToLong(item -> (long) item.getQty() * item.getProductOption().getPrice())
				.sum();
...
//		}
		...
		Payment payment = Payment.builder().cart(cart).payMethod(requestDTO.getPayMethod()).payStatus("COMPLETED")
				.totalAmount(totalAmount).paymentDate(LocalDate.now()).build();
		Payment savedPayment = paymentRepository.save(payment);

		// 6. PaymentDetail 테이블에 상세 내역 기록
		for (CartItem cartItem : cartItems) {
			PaymentDetail paymentDetail = PaymentDetail.builder().payment(savedPayment).productOption(cartItem.getProductOption())
					.quantity(cartItem.getQty()).price(cartItem.getProductOption().getPrice()).pno(cartItem.getProductOption().getProduct().getPno()).build();
